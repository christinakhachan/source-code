    ...
    <section className="py-24 px-6 sm:px-12 bg-stone-100">
      <div className="max-w-5xl mx-auto">
        <motion.div
          ...
          className="mb-16"
        >
          <h2 className="text-4xl sm:text-5xl font-['Playfair_Display',serif] text-stone-900 mb-4">Professional Experience</h2>
          <p className="text-stone-500 text-lg font-light">Building foundations in diverse, fast-paced environments.</p>
        </motion.div>
          ...
          {experiences.map((exp, index) => (
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <div className={`p-3 bg-white rounded-2xl shadow-sm ${exp.iconColor}`}>
                  <Briefcase className="w-6 h-6" />
                </div>
                <div>
                  <span className={`text-sm font-medium ${exp.iconColor}`}>{exp.date}</span>
              ...
              <ul className="space-y-3">
                {exp.bullets.map((bullet, i) => (
                  <li key={i} className="flex items-start gap-3 text-stone-600 font-light">
                    <span className={`mt-2 w-1.5 h-1.5 rounded-full shrink-0 ${exp.iconColor.replace('text', 'bg')}`} />
